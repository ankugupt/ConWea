...
import pickle 
with open('/content/df_contextualized10000end.pkl', 'rb') as f:
...
import pandas as pd
df_train = df.truncate(before=0,after=2100)
df_test = df.truncate(before=2101,after=2500)
...
def cluster_GMM(num_clusters, word_vectors):
    ...
    idx = joblib.load("/content/gmm_latestclusmodel_len2alldata.pkl")
    idx_proba = joblib.load("/content/gmm_prob_latestclusmodel_len2alldata.pkl")
    return (idx,idx_proba)
    # Initalize a GMM object and use it for clustering.
    clf = GaussianMixture(n_components=num_clusters,
                          covariance_type="tied", init_params='kmeans', max_iter=120,verbose=3)
...
if __name__ == '__main__':
    ...
    all= all.truncate(before=0,after=2500)
    ...
    for idx, vec in enumerate(word_vectors):
      if len(vec)!=768:
        word_vectors[idx]=word_vectors[24718]
    ...
    word_idf_dict = calculate_inv_doc_freq(df,calculate_df_doc_freq(all))
